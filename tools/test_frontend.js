...
test("navigation controls use lightweight Liquid Glass with accessible fallbacks", () => {
  ...
  assert.match(stylesheet, /:root\s*\{[^}]*--header-bg:\s*rgba\(247, 248, 250, 0\.97\);[^}]*--category-track-bg:\s*rgba\(255, 255, 255, 0\.82\);[^}]*--category-track-bg-fallback:\s*#ffffff;[^}]*--category-track-border:\s*rgba\(37, 35, 56, 0\.1\);[^}]*--category-track-shadow:[^}]*--category-selected-bg:\s*rgba\(226, 226, 229, 0\.9\);/s);
...
test("the public access notice stays compact and responds to available width", () => {
  ...
  assert.match(stylesheet, /\.access-notice\s*\{[^}]*border:\s*1px solid var\(--notice-border\);[^}]*background:\s*var\(--notice-bg\);[^}]*box-shadow:\s*none;/s);
  assert.match(stylesheet, /\.access-notice i\s*\{[^}]*color:\s*var\(--primary\);/s);
